...
interface PagamentoActions {
  ...
  listarPagamentos: (filtros?: FiltrosPagamento) => Promise<void>;
  atualizarStatusPagamento: (id: number, dados: AtualizarPagamento) => Promise<void>;
  
  // Métodos de pagamento
  carregarMetodosUsuario: (usuarioId: number) => Promise<void>;
  definirMetodoPrincipal: (metodo: MetodoPagamentoUsuario) => void;
  
  // Filtros e Estado
  atualizarFiltros: (novosFiltros: Partial<FiltrosPagamento>) => void;
  limparFiltros: () => void;
  limparErro: () => void;
  
  // Utilitários
  recarregarDados: (usuarioId?: number) => Promise<void>;
}
...
export function PagamentoProvider({ children }: PagamentoProviderProps) {
  ...
  const [filtros, setFiltros] = useState<FiltrosPagamento>(initialState.filtros);
  ...
  const listarPagamentos = async (novosFiltros?: FiltrosPagamento): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const carregarMetodosUsuario = async (usuarioId: number): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const atualizarFiltros = (novosFiltros: Partial<FiltrosPagamento>): void => {
  ...
  const recarregarDados = async (usuarioId?: number): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
